...
// AccountGetPhotosResponse ..
type AccountGetPhotosResponse struct {
	Count int `json:"count"`
	Items []UserPhoto
}
...
// GetBanned returns a user's blacklist.
// https://vk.com/dev/account.getBanned
func (a *Account) GetPhotos(userID uint) (AccountGetPhotosResponse, error) {
	params := map[string]string{
		"owner_id": fmt.Sprint(userID),
		"album_id": "profile",
		"count":    "10",
	}
	resp, err := a.vk.Request("photos.get", params)
	if err != nil {
		return AccountGetPhotosResponse{}, err
	}
	var list AccountGetPhotosResponse
	err = json.Unmarshal(resp, &list)
	if err != nil {
		return AccountGetPhotosResponse{}, err
	}
...
// Get returns current account info.
// https://vk.com/dev/users.get
func (a *Account) Get(fields string) (UserObject, error) {
	params := map[string]string{"fields": fields}
	resp, err := a.vk.Request("users.get", params)
	var info []UserObject
	var info2 UserObject
	if err != nil {
		return info2, err
	}
	err = json.Unmarshal(resp, &info)
